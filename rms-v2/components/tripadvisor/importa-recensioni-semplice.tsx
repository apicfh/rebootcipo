...
interface ImportaRecensioniSempliceProps {
  hotelId: string
  onSuccess?: (count?: number) => void
  disabled?: boolean
}

export function ImportaRecensioniSemplice({ hotelId, onSuccess, disabled = false }: ImportaRecensioniSempliceProps) {
  ...
  const importaRecensioni = async () => {
    ...
    try {
      const response = await fetch("/api/tripadvisor/importa-recensioni-hotel", {
        method: "POST",
        headers: {
