...
-- Ricreiamo con logica di filtraggio corretta
...
DECLARE
    ...
    settimana_fine DATE;
BEGIN
    -- Ottieni i dettagli della settimana selezionata
    ...
    -- LOGICA CORRETTA: Cerca prenotazioni per quella settimana di soggiorno
    prenotazioni_aggregate AS (
        ...
        WHERE pr.id_hotel = p_hotel_id
          ...
          -- NUOVO FILTRO: Prenotazioni che hanno soggiorno nella settimana selezionata
          AND (
              -- Opzione 1: Se la prenotazione ha la settimana nell'array settimane
              settimana_nome = ANY(pr.settimane)
              OR
              -- Opzione 2: Se le date di soggiorno si sovrappongono con la settimana
              (pr.arrivo <= settimana_fine AND pr.partenza >= settimana_inizio)
              OR  
              -- Opzione 3: Se la stagione corrisponde (per prenotazioni senza settimane specifiche)
              (pr.stagione = p_stagione AND (pr.settimane IS NULL OR array_length(pr.settimane, 1) = 0))
          )
