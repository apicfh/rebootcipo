...
-- Poi la ricreiamo con debug per verificare l'aggregazione
...
BEGIN
    ...
    IF settimana_nome IS NULL THEN
        RETURN QUERY SELECT 
            ...
            JSONB_BUILD_OBJECT('error', 'Settimana non trovata') as debug_info;
        RETURN;
    END IF;

    -- Conta le prenotazioni totali prima dell'aggregazione
    SELECT COUNT(*) INTO total_prenotazioni
    ...
    SELECT 
        ...
        JSONB_BUILD_OBJECT(
            ...
            'prenotazioni_totali', total_prenotazioni,
            'settimane_aggregate', COALESCE(prd.settimane_count, 0),
            'somma_prenotazioni_aggregate', (
                SELECT SUM((value->>'numero_prenotazioni')::INTEGER) 
