-- Funzione ottimizzata per calcolare il tasso di conversione
...
LANGUAGE plpgsql
SECURITY DEFINER
SET statement_timeout = '60s'  -- Aumentiamo il timeout a 60 secondi
AS $function$
DECLARE
  result json;
BEGIN
  -- Utilizziamo una query più efficiente con indici e limitazioni
  ...
  -- Aggregazione per mese con conteggi ottimizzati
...
-- Imposta i permessi
COMMENT ON FUNCTION public.get_conversion_rate IS 'Calcola il tasso di conversione per periodo (ottimizzata)';
